    ...
    def use_power(self):
        for member in self.members:
            if member["Age"] > 18:
                ...
                raise Exception("No powers under 18")
            
    def incredible_presentation(self):
        for member in self.members:
            power = member.get("power", "No Power")
            super_name = member.get("super_name", "Unknown")
            print(f"This is our incredible family!: {last_name} - {member["Name"]}, {member["Age"]}, {member["Gender"]}, {power}, {super_name}")
...
incredibles_family.born(Name="Baby Jack", Age=0, Gender="Male", power="Unknown Power", super_name="Unknown Super name")
